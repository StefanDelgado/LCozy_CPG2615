<?php
// /Main/modules/student/student_management.php
...
// -----------------------------
// Fetch Students (3 groups by verified state)
// -----------------------------
...
$pendingStmt = $pdo->prepare("
    ...
    FROM users
    WHERE role = 'student' AND verified = 0
...
$rejectedStudents = $rejectedStmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Student Management — Super Admin</title>
...
<div class="top-nav">
  <h3 style="margin:0;">Super Admin — Student Management</h3>
  <div>
...
<div class="card">
      ...
      <table>
        ...
        <?php foreach ($pendingStudents as $s): ?>
          <tr>
            ...
            <td>
              <?php if ($s['id_document']): ?>
                <a class="btn-view" href="/uploads/<?= htmlspecialchars($s['id_document']) ?>" target="_blank">View</a>
              <?php else: ?>
            ...
            <td>
              <a class="btn btn-success" href="process_student_request.php?id=<?= $s['user_id'] ?>&action=approve">Approve</a>
              <a class="btn btn-danger" href="process_student_request.php?id=<?= $s['user_id'] ?>&action=disapprove">Disapprove</a>
            </td>
          </tr>
        <?php endforeach; ?>
        </tbody>
  ...
  <div id="tab-verified" class="tab" style="display:none;">
    ...
    <?php if (empty($verifiedStudents)): ?>
      <p class="muted">No verified students.</p>
    <?php else: ?>
      <table>
        <thead>
        ...
        <tbody>
        <?php foreach ($verifiedStudents as $s): ?>
          <tr>
            <td><?= htmlspecialchars($s['name']) ?></td>
            <td><?= htmlspecialchars($s['email']) ?></td>
            <td><?= htmlspecialchars($s['phone'] ?? '—') ?></td>
  ...
  <div id="tab-rejected" class="tab" style="display:none;">
    <h3>Disapproved Students</h3>

    <?php if (empty($rejectedStudents)): ?>
      <p class="muted">No disapproved students.</p>
    <?php else: ?>
      <table>
        <thead>
